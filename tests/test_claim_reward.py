...
class Delegator:
    ...
    def old_claim_btc_reward(self):
        ...
        print('old_claim_btc_reward>>>>>>>>', tx.events)
        return tx
...
class TestInitHardForkDelegate:
    def test_init_hard_fork_delegate0(self, pledge_agent, candidate_hub):
        ...
        lock_time, end_round = mock_btc_stake_lock_time(timestamp)
        old_turn_round()
    ...
    def test_init_hard_fork_delegate1(self, pledge_agent, btc_agent, stake_hub, candidate_hub, core_agent, btc_stake):
        ...
        set_round_tag(init_round_tag)
        delegators = add_delegators()
        operators, consensuses = add_candidates(3)
        old_turn_round()
        timestamp = get_current_round() * Utils.ROUND_INTERVAL
        lock_time, end_round = mock_btc_stake_lock_time(timestamp, 10)
        tx_ids = []
        ...
        for delegator in delegators:
            print('address', delegator.get_address())
            delegator.claim_reward()
        turn_round(consensuses)
        print('dsafasfafa', get_current_round())
        print(CandidateHubMock[0].roundTag())
        print(BitcoinStakeMock[0].roundTag())
        ...
        for delegator in delegators:
            for op in operators:
                ...
                print('getDelegator', core_agent.getDelegator(op, delegator.get_address()))
                print('getDelegatorMap', core_agent.getDelegatorMap(delegator.get_address()))
        btc_stake.moveData(tx_ids)
        for tx in tx_ids:
            print('btc_stake', btc_stake.receiptMap(tx))
    ...
    def test_move_btc_no_reward(self, btc_agent, pledge_agent, stake_hub, candidate_hub, core_agent, btc_stake):
        ...
        set_round_tag(init_round_tag)
        delegators = add_delegators()
        operators, consensuses = add_candidates(1)
        delegators[0].old_delegate_coin(operators[0], DELEGATE_VALUE)
        old_turn_round()
        timestamp = get_current_round() * Utils.ROUND_INTERVAL
        lock_time, end_round = mock_btc_stake_lock_time(timestamp, 10)
        tx_ids = []
        tx_id = delegators[0].old_delegate_btc(operators[0], BTC_VALUE, lock_time)
        tx_ids.append(tx_id)
        old_turn_round(consensuses)
        old_turn_round(consensuses)
        delegators[0].old_claim_btc_reward()
    ...
    def test_move_btc_no_reward2(self, btc_agent, pledge_agent, stake_hub, candidate_hub, core_agent, btc_stake):
        ...
        # delegators[0].old_claim_btc_reward()
        # delegators[0].old_claim_reward(operators)
        # delegators[0].old_claim_btc_reward()
...
class TestClaimReward:
    ...
    def test_claim_reward_after_hardcap_update(self, stake_hub, hard_cap, set_candidate):
        ...
        turn_round()
        delegate_coin_success(operators[0], COIN_VALUE, accounts[0])
