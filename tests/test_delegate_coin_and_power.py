...
def test_distribute_power_reward_during_turn_round(core_agent, hash_power_agent, stake_hub, btc_light_client,
    ...
    assert tx.events['roundReward'][0]['amount'] == collateral_reward['coin'][operators[0]]
    assert tx.events['roundReward'][1]['amount'] == collateral_reward['coin'][operators[1]]
    assert tx.events['roundReward'][2]['amount'] == collateral_reward['power'][operators[0]]
    assert tx.events['roundReward'][3]['amount'] == collateral_reward['power'][operators[1]]
