...
PAY_ADDRESS1 = "0xa914c0958c8d9357598c5f7a6eea8a807d81683f9bb687"
...
def test_delegate_btc_success_public_hash(btc_stake, set_candidate):
    ...
    btc_tx = build_btc_tx(operators[0], accounts[0], BTC_VALUE, lock_script)
    print('fdsafafad', lock_script)
    print('fdsafafad', btc_tx)
    tx = btc_stake.delegate(btc_tx, 0, [], 0, lock_script)
    tx_id = get_transaction_txid(btc_tx)
    turn_round()
    expect_event(tx, 'delegated', {
...
def test_move_data_success(btc_stake, pledge_agent, set_candidate):
    ...
    end_round = LOCK_TIME // Utils.ROUND_INTERVAL
    tx_id = __mock_delegate_btc_old(btc_amount, operators[2], accounts[1])
    ...
    tx_ids = []
    for i in range(2):
        tx_id = __mock_delegate_btc_old(btc_amount + i, operators[i], accounts[i])
        tx_ids.append(tx_id)
...
def test_move_data_with_btc_already_collateralized(btc_stake, pledge_agent, set_candidate):
    operators, consensuses = set_candidate
    btc_amount = BTC_VALUE // 2
    tx_id = __mock_delegate_btc_old(btc_amount, operators[1], accounts[1])
    btc_stake.moveData([tx_id])
    __check_receipt_map_info(tx_id, {
        'candidate': operators[1],
        'delegator': accounts[1],
...
def __get_round2_expire_info_map(round_tag):
    ...
    print('__get_round2_expire_info_map>>>>>>>> agents', agents)
    print('__get_round2_expire_info_map>>>>>>>> amounts', amounts)
...
def __mock_delegate_btc_old(btc_value, agent, delegator, tx_id=None, script=LOCK_SCRIPT, lock_time=LOCK_TIME, fee=1):
    if tx_id is None:
        tx_id = random_btc_tx_id()
    tx = PLEDGE_AGENT.delegateBtcMock(tx_id, btc_value, agent, delegator, script, lock_time, fee)
    assert 'delegatedBtcOld' in tx.events
    return tx_id
...
def __get_accured_reward_per_btc_map(validate, round_tag):
    ...
    print('__get_accured_reward_per_btc_map>>>>>>>', data)
    return data
