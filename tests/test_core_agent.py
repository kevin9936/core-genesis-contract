...
def test_add_round_reward_success(core_agent, validator_set, candidate_hub, stake_hub, btc_agent, btc_lst_stake,
    ...
    for index, t in enumerate(tx.events['roundReward']):
        ...
        if t['name'] == Web3.keccak(text='CORE').hex():
            assert t['amount'] == collateral_reward['coin'][t['validator']]
            assert t['bonus'] == 0
        # power
        elif t['name'] == Web3.keccak(text='HASHPOWER').hex():
            assert t['amount'] == collateral_reward['power'][t['validator']]
            assert t['bonus'] == 0
        # btc
        elif t['name'] == Web3.keccak(text='BTC').hex():
            assert t['amount'] == collateral_reward['btc'][t['validator']]
            assert t['bonus'] == 0
...
def test_delegate_coin_success(core_agent):
    ...
    result = core_agent.getDelegator(agent, delegator).dict()
    __check_coin_delegator(result, required_coin_deposit + deposit, required_coin_deposit + deposit * 2, round_tag, 0)
...
def test_undelegate_coin_success(core_agent):
    ...
    __check_coin_delegator(core_agent.getDelegator(agent, delegator).dict(), 0, 0, 0, 0)
...
def test_transfer_coin_success(core_agent):
    ...
    round_tag = core_agent.roundTag()
    result = core_agent.getDelegator(agent_source, delegator).dict()
    __check_coin_delegator(result, 0, required_coin_deposit, round_tag, 0)
    turn_round()
    round_tag = core_agent.roundTag()
    ...
    __check_coin_delegator(core_agent.getDelegator(agent_source, delegator), 0, 0, round_tag, required_coin_deposit)
    __check_coin_delegator(core_agent.getDelegator(agent_target, delegator), 0, required_coin_deposit, round_tag, 0)
...
def test_transfer_coin_failed_with_inactive_target_agent(core_agent):
    ...
    __delegate_coin_success(agent_source, delegator, 0, required_coin_deposit)
    round_tag = core_agent.roundTag()
    __check_coin_delegator(core_agent.getDelegator(agent_source, delegator).dict(), 0, required_coin_deposit,
                           round_tag, 0)
...
def test_remaining_stake_includes_current_round_stake(core_agent, validator_set, operate):
    ...
    if operate == 'undelegate':
        tx = core_agent.undelegateCoin(operators[0], undelegate_amount, {'from': accounts[0]})
        event = 'undelegatedCoin'
        delegate_info = __get_delegator_info(operators[0], accounts[0])
        __check_coin_delegator(delegate_info, MIN_INIT_DELEGATE_VALUE - 1, delegate_amount * 2 - undelegate_amount,
                               get_current_round(), 0)
        with brownie.reverts("Not enough staked tokens"):
        ...
        tx = core_agent.transferCoin(operators[0], operators[1], undelegate_amount, {'from': accounts[0]})
        event = 'transferredCoin'
        delegate_info0 = __get_delegator_info(operators[0], accounts[0])
        delegate_info1 = __get_delegator_info(operators[1], accounts[0])
        __check_coin_delegator(delegate_info0, MIN_INIT_DELEGATE_VALUE - 1, delegate_amount * 2 - undelegate_amount,
                               get_current_round(), undelegate_amount)
        __check_coin_delegator(delegate_info1, 0, undelegate_amount,
                               get_current_round(), 0)
        with brownie.reverts("Not enough staked tokens"):
            core_agent.transferCoin(operators[0], operators[1], required_coin_deposit, {'from': accounts[0]})
...
def __check_coin_delegator(c_delegator, staked_amount, real_amount, change_round, transferred_amount):
    assert c_delegator['stakedAmount'] == staked_amount
    assert c_delegator['realtimeAmount'] == real_amount
    assert c_delegator['changeRound'] == change_round
    assert c_delegator['transferredAmount'] == transferred_amount
