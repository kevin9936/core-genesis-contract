...
class ChainHandler:
    ...
    def select_validators_for_next_round(self):
        ...
        for asset in assets:
            ...
            for candidate in available_candidates.values():
                amount_list = asset.calc_candidate_stake_amount_list(candidate, delegator_stake_state, self.chain.get_round())
    ...
    def add_candidate(self, operator_addr, consensus_addr, fee_addr, commission, margin):
        ...
        validators = self.chain.get_validators()
        round = self.chain.get_round()
        assert candidates.get(operator_addr) is None
    ...
    def claim_reward(self, delegator):
        ...
        core_asset_reward = 0
        for asset in assets:
            claimable_reward, unclaimable_reward = asset.claim_reward(
                round,
                delegator,
            ...
            if asset == assets[0]:
                core_asset_reward = claimable_reward
            ...
            claimable_reward, unclaimable_reward = \
                asset.apply_dual_stake_reward(claimable_reward, unclaimable_reward, delegator_stake_state, core_asset_reward)
    ...
    def delegate_core(self, delegator, delegatee, amount, is_transfer=False):
        ...
        if change_round < round:
            reward = core_asset.collect_reward_in_candidate(round, delegator, candidate)
            if reward > 0:
    ...
    def transfer_btc(self, delegator, txid, to_delegatee):
        ...
        claimable_reward, unclaimable_reward, expired = btc_asset.collect_btc_stake_tx_reward(tx, delegator_stake_state, round)
    ...
    def delegate_btc_lst(self, tx):
        ...
        if change_round < round:
            ...
            reward = btc_asset.collect_btc_lst_reward(round, delegator, delegator_stake_state)
            if reward > 0:
    ...
    def redeem_btc_lst(self, delegator, amount, payment):
        ...
        history_reward = btc_asset.collect_btc_lst_reward(round, delegator, delegator_stake_state)
        if history_reward > 0:
