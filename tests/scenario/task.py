...
class RegisterCandidate(Task):
    ...
    def pre_execute(self, params):
        ...
        self.margin = int(params[2] * constants.CORE_DECIMALS) if len(params) == 3 else CandidateHubMock[0].requiredMargin()
...
class AddMargin(Task):
    def pre_execute(self, params):
        ...
        self.operator_addr = get_operator_addr(params[0])
        self.amount = int(params[1] * constants.CORE_DECIMALS) if len(params) == 2 else CandidateHubMock[0].requiredMargin()
