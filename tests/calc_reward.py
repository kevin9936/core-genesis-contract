...
def parse_delegation(agents, block_reward, power_factor=500, btc_factor=10):
        ...
        for agent in agents:
            ...
            if s == 'coin':
                ...
                agent['btc_reward'] = total_reward * (agent['total_btc'] * btc_factor) // agent[
                    'validator_score'] * collateral_state[s] // DENOMINATOR
                agent['single_btc_reward'] = agent['btc_reward'] // agent['total_btc']
    ...
    for agent in agents:
        ...
        for item in agent_btc:
            ...
            actual_account_btc_reward = agent['single_btc_reward'] * item['value']
            ...
            print(f"btc reward11: {agent['address']} on {item['address']} => {actual_account_btc_reward}")
        for item in agent_coin:
            reward = agent['coin_reward'] * (item['value'] - item['undelegate_amount']) // agent['total_coin']
    ...
    for a in agents:
        print('agent Reward', a['coin_reward'])
