...
def test_delegate_btc_with_lock_time_in_tx(btc_stake, set_candidate, stake_hub, btc_lst_stake, btc_agent):
    ...
    print('delegatorMap', btc_stake.getDelegatorBtcMap(accounts[0]))
    ...
    tx = stake_hub.getDebts(accounts[0])
    print('getDebts', tx)
...
def test_delegate_btc_with_lock_script_in_tx(btc_stake, set_candidate):
    ...
    assert tracker0.delta() == BTC_REWARD1 - FEE
    assert tracker1.delta() == FEE
...
def test_delegate_btc_success_public_key(btc_stake, set_candidate):
    ...
    assert tracker.delta() == BTC_REWARD
...
def test_collect_porter_fee_success(btc_stake, set_candidate, delegate_btc_valid_tx):
    ...
    assert btc_stake.btcReceiptMap(tx_id)['fee'] == 0
    tx = btc_stake.claimBtcReward(tx_id_list)
...
def test_deduct_multiple_porter_fees_when_claiming_rewards(btc_stake, set_candidate, delegate_btc_valid_tx):
    operators, consensuses = set_candidate
    lock_script, delegate_btc_tx0, tx_id_list = delegate_btc_valid_tx
    round_tag = get_current_round()
    tx_id0 = tx_id_list[0]
    ...
    btc_delegator0 = btc_stake.btcReceiptMap(tx_id0)
    btc_delegator1 = btc_stake.btcReceiptMap(tx_id1)
    ...
    assert btc_stake.btcReceiptMap(tx_id0)['fee'] == btc_stake.btcReceiptMap(tx_id1)['fee'] == 0
...
def test_claim_rewards_and_deduct_porter_fees_after_transfer(btc_stake, set_candidate, delegate_btc_valid_tx):
    ...
    tracker1 = get_tracker(accounts[1])
    btc_stake.claimBtcReward(tx_id_list)
    btc_stake.claimReward(operators, {'from': accounts[1]})
    assert tracker0.delta() == 0
    assert tracker1.delta() == total_reward - total_reward // 2
    assert btc_stake.btcReceiptMap(tx_id_list[0])["fee"] == FEE
    turn_round(consensuses)
    btc_stake.claimReward(operators, {'from': accounts[1]})
    btc_stake.claimBtcReward(tx_id_list)
...
def test_claim_rewards_with_insufficient_porter_funds(btc_stake, set_candidate, delegate_btc_valid_tx):
    ...
    tracker1 = get_tracker(accounts[1])
    tx = btc_stake.claimBtcReward([tx_id])
    voucher = btc_stake.btcReceiptMap(tx_id)
    expect_event(tx, "transferredBtcFee", {
        "txid": tx_id,
        "feeReceiver": accounts[1],
        "fee": BLOCK_REWARD // 2
...
def test_claim_rewards_success_with_max_stake_certificates(btc_stake, set_candidate, delegate_btc_valid_tx):
    operators, consensuses = set_candidate
    end_round = lock_time // ROUND_INTERVAL
    btc_stake.setClaimRoundLimit(10)
    set_last_round_tag(end_round - MIN_BTC_LOCK_ROUND - 1)
...
def test_distribute_rewards_to_multiple_addresses(btc_stake, set_candidate):
    ...
    tracker0 = get_tracker(accounts[0])
    tracker1 = get_tracker(accounts[1])
    btc_stake.claimBtcReward(tx_id_list1)
    btc_stake.claimBtcReward(tx_id_list0, {"from": accounts[1]})
    assert tracker0.delta() == BLOCK_REWARD // 4 + FEE
    assert tracker1.delta() == BLOCK_REWARD // 2 - BLOCK_REWARD // 4 - FEE


def test_claim_rewards_for_multiple_coin_staking(btc_stake, set_candidate):
    total_reward = BLOCK_REWARD // 2
    operators, consensuses = set_candidate
    lock_script = get_lock_script(lock_time, public_key, lock_script_type)
    ...
    btc_stake.delegateCoin(operators[0], {"value": delegate_amount, "from": accounts[2]})
    btc_stake.delegateCoin(operators[1], {"value": delegate_amount // 2})
    tx_id_list0, tx_id_list1 = [], []
...
def test_multiple_users_staking_to_same_validator(btc_stake, set_candidate):
    ...
    tracker0 = get_tracker(accounts[0])
    tracker1 = get_tracker(accounts[1])
    btc_stake.claimBtcReward(tx_id_list0)
    btc_stake.claimBtcReward(tx_id_list1, {"from": accounts[1]})
...
def test_claim_rewards_for_multiple_stakes_to_different_validators(btc_stake, set_candidate, delegate_btc_valid_tx):
    ...
    btc_stake.delegate(delegate_btc_tx1, 0, [], 0, lock_script)
    btc_stake.delegateCoin(operators[0], {"value": delegate_amount, "from": accounts[2]})
    btc_stake.delegateCoin(operators[1], {"value": delegate_amount, "from": accounts[2]})
    turn_round()
    turn_round(consensuses, round_count=2)
    tracker0 = get_tracker(accounts[0])
    btc_stake.claimBtcReward(tx_id_list)
...
def test_claim_rewards_after_modifying_btc_factor(btc_stake, set_candidate, delegate_btc_valid_tx, btc_factor):
    btc_stake.setBtcFactor(btc_factor)
    btc_factor = btc_factor * btc_stake.BTC_UNIT_CONVERSION()
    total_reward = BLOCK_REWARD // 2
...
def test_transfer_btc_when_no_rewards_in_current_round(btc_stake, set_candidate, delegate_btc_valid_tx):
    ...
    voucher0 = btc_stake.btcReceiptMap(tx_id_list[0])
    assert voucher0["endRound"] == end_round
    assert btc_stake.agentsMap(operators[0])["btc"] == btc_stake.agentsMap(operators[0])["totalBtc"] == 0
    assert btc_stake.agentsMap(operators[1])["btc"] == btc_stake.agentsMap(operators[1])["totalBtc"] == BTC_VALUE
...
def test_claim_rewards_with_different_fees_success(btc_stake, set_candidate, fee):
    ...
    turn_round(consensuses, round_count=2)
    tracker1 = get_tracker(accounts[1])
    tx = btc_stake.claimBtcReward([tx_id0])
    if fee > 0:
        expect_event(tx, 'transferredBtcFee', {
            'txid': tx_id0,
            'fee': fee * btc_stake.FEE_FACTOR(),
            'feeReceiver': accounts[1]
        })
    else:
        assert 'transferredBtcFee' not in tx.events
    assert tracker1.delta() == fee * btc_stake.FEE_FACTOR()


def test_insufficient_rewards_to_pay_porter_fee(btc_stake, set_candidate):
    end_round = lock_time // ROUND_INTERVAL
    fee = 100
    actual_fee = fee * btc_stake.FEE_FACTOR()
    delegate_amount = 20000
    operators, consensuses = set_candidate
    set_last_round_tag(end_round - MIN_BTC_LOCK_ROUND - 1)
    lock_script = get_lock_script(lock_time, public_key, lock_script_type)
...
def test_multiple_reward_transfers_in_multiple_rounds(btc_stake, set_candidate, delegate_btc_valid_tx):
    ...
    for index, operator in enumerate(operators):
        ...
        transfer_voucher = btc_stake.btcReceiptMap(tx_id_list[index])
        ...
        latest_voucher = btc_stake.btcReceiptMap(tx_id_list[index])
    ...
    btc_stake.setClaimRoundLimit(3)
    turn_round(consensuses, round_count=2)
    tracker0 = get_tracker(accounts[0])
    btc_stake.claimBtcReward(tx_id_list)
    ...
    btc_stake.setClaimRoundLimit(7)
    btc_stake.claimBtcReward(tx_id_list)
    assert tracker0.delta() == BLOCK_REWARD // 2 * 7 - FEE * 7
    turn_round(consensuses, round_count=2)
    btc_stake.setClaimRoundLimit(21)
    btc_stake.claimBtcReward(tx_id_list)
    """
    ...
    assert btc_stake.btcReceiptMap(tx_id_list[0])['agent'] == ZERO_ADDRESS
    btc_stake.claimBtcReward(tx_id_list[-2:])
    assert tracker0.delta() == BLOCK_REWARD // 2 * 3 - FEE


def test_claim_limit_exhausted(btc_stake, set_candidate, delegate_btc_valid_tx):
    operators, consensuses = set_candidate
    end_round = lock_time // ROUND_INTERVAL
    set_last_round_tag(end_round - MIN_BTC_LOCK_ROUND - 1)
    lock_script = get_lock_script(lock_time, public_key, lock_script_type)
    ...
    btc_stake.setClaimRoundLimit(10)
    delegate_amount = 20000
    btc_stake.delegateCoin(operators[1], {"value": delegate_amount, "from": accounts[1]})
    btc_stake.delegateCoin(operators[2], {"value": delegate_amount, "from": accounts[1]})
    turn_round(consensuses, round_count=2)
    btc_stake.claimBtcReward(tx_id_list)
    turn_round(consensuses, round_count=2)
    ...
    assert btc_stake.btcReceiptMap(tx_id_list[2])['agent'] == ZERO_ADDRESS
...
def test_claiming_reward_with_unconfirmed_txid_reverts(btc_stake, set_candidate, delegate_btc_valid_tx):
    operators, consensuses = set_candidate
    end_round = lock_time // ROUND_INTERVAL
    set_last_round_tag(end_round - MIN_BTC_LOCK_ROUND - 1)
    lock_script, btc_tx, tx_id_list = delegate_btc_valid_tx
    ...
    with brownie.reverts("btc tx not found"):
        btc_stake.claimBtcReward([error_tx_id])
    error_tx_id = '0x0'
    with brownie.reverts("btc tx not found"):
        btc_stake.claimBtcReward([error_tx_id])
...
def test_modify_btc_factor_after_delegating_btc(btc_stake, set_candidate, delegate_btc_valid_tx):
    end_round = lock_time // ROUND_INTERVAL
    total_reward = BLOCK_REWARD // 2
    delegate_amount = 20000
    operators, consensuses = set_candidate
...
def test_stake_multiple_currencies_and_claim_rewards(btc_stake, candidate_hub, btc_light_client, set_candidate,
    ...
    total_reward = BLOCK_REWARD // 2
    operators, consensuses = set_candidate
    btc_stake.setPowerBlockFactor(100000)
    turn_round()
...
def test_claiming_btc_reward_with_multiple_power(btc_stake, candidate_hub, btc_light_client, set_candidate,
    ...
    btc_stake.claimReward(operators, {'from': accounts[1]})
    btc_stake.claimReward([], {'from': accounts[2]})
    btc_stake.claimReward([], {'from': accounts[3]})
    total_reward = BLOCK_REWARD // 2
...
def test_claim_coin_reward_correctly_with_existing_btc_staking(btc_stake, candidate_hub, set_candidate,
                                                               delegate_btc_valid_tx):
    end_round = lock_time // ROUND_INTERVAL
    total_reward = BLOCK_REWARD // 2
    delegate_amount = 20000
...
def test_coin_transfer_with_power_and_btc_staking(btc_stake, btc_light_client, set_candidate, delegate_btc_valid_tx,
    ...
    tx = btc_stake.transferCoin(operators[0], operators[2], transfer_amount, {'from': accounts[1]})
...
def test_undelegate_with_power_and_btc_staking(btc_stake, set_candidate, btc_light_client, delegate_btc_valid_tx,
    ...
    btc_stake.undelegateCoin(operators[0], undelegate_amount, {'from': accounts[1]})
    tx = turn_round(consensuses)
...
def test_claiming_historical_rewards_with_btc_transfer(btc_stake, set_candidate, delegate_btc_valid_tx):
    end_round = lock_time // ROUND_INTERVAL
    delegate_amount = 20000
    operators, consensuses = set_candidate
    set_last_round_tag(end_round - MIN_BTC_LOCK_ROUND - 3)
...
def test_transfer_btc_to_existing_btc_staker(btc_stake, set_candidate, delegate_btc_valid_tx):
    ...
    tracker0 = get_tracker(accounts[0])
    tracker1 = get_tracker(accounts[1])
    btc_stake.claimBtcReward([tx_id_list[0]], {'from': accounts[0]})
    btc_stake.claimBtcReward([tx_id1], {'from': accounts[1]})
    total_reward = BLOCK_REWARD // 2
...
def test_claim_reward_reentry(btc_stake, set_candidate, delegate_btc_valid_tx):
    btc_stake_proxy = ClaimBtcRewardReentry.deploy(btc_stake.address, {'from': accounts[0]})
    end_round = lock_time // ROUND_INTERVAL
    operators, consensuses = set_candidate
    set_last_round_tag(end_round - MIN_BTC_LOCK_ROUND - 3)
    lock_script, _, _ = delegate_btc_valid_tx
    delegate_btc_tx1, tx_id1 = get_btc_tx(BTC_VALUE, chain_id, operators[0], btc_stake_proxy.address,
                                          lock_script_type)
    btc_stake.delegate(delegate_btc_tx1, 0, [], 0, lock_script, {'from': accounts[2]})
    btc_stake.delegateCoin(operators[0], {"value": 20000, "from": accounts[1]})
    turn_round()
    btc_stake_proxy.setTxidList([tx_id1])
    turn_round(consensuses)
    tracker = get_tracker(btc_stake_proxy)
    tx = btc_stake_proxy.claimBtcReward([tx_id1])
...
def test_transfer_btc_reentry(btc_stake, set_candidate, delegate_btc_valid_tx):
    btc_stake_proxy = TransferBtcReentry.deploy(btc_stake.address, {'from': accounts[0]})
    end_round = lock_time // ROUND_INTERVAL
    operators, consensuses = set_candidate
    set_last_round_tag(end_round - MIN_BTC_LOCK_ROUND - 3)
    lock_script, _, _ = delegate_btc_valid_tx
    ...
    btc_stake.delegateCoin(operators[0], {"value": 20000, "from": accounts[1]})
    turn_round()
    btc_stake_proxy.setTxid(tx_id1)
...
def test_update_param_success(btc_stake, gov_hub, key, value):
    ...
    if key == 'requiredCoinDeposit':
        assert btc_stake.requiredCoinDeposit() == value
    elif key == 'powerFactor':
        assert btc_stake.powerFactor() == value
    elif key == 'btcFactor':
        assert btc_stake.btcFactor() == value
    elif key == 'minBtcLockRound':
        assert btc_stake.minBtcLockRound() == value
    elif key == 'btcConfirmBlock':
        assert btc_stake.btcConfirmBlock() == value
    elif key == 'delegateBtcGasPrice':
        assert btc_stake.delegateGasPrice() == value
    else:
        assert btc_stake.minBtcValue() == value
...
def test_revert_for_too_high_gas_price(btc_stake, delegate_btc_valid_tx, set_candidate, relay_hub, gov_hub):
    lock_script, btc_tx, tx_id_list = delegate_btc_valid_tx
    delegate_gas_price = btc_stake.INIT_DELEGATE_BTC_GAS_PRICE() + 1
