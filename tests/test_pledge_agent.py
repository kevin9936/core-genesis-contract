# test the old data migration logic
...
def test_migration_scenario_1314(pledge_agent, validator_set, stake_hub):
    operators, consensuses = __register_candidates(accounts[2:5])
    __old_delegate_coin(operators[0])
    __old_delegate_coin(operators[1])
    __old_delegate_coin(operators[2])
    __get_old_agent_map_info(operators[0])
    __get_old_agent_map_info(operators[1])
    __get_old_agent_map_info(operators[2])
    __old_turn_round()
    __init_hybrid_score_mock()
    __old_turn_round(consensuses)
    __old_undelegate_coin(operators[0])
    __old_undelegate_coin(operators[1])
    __old_transfer_coin(operators[2], operators[0])
    __old_turn_round(consensuses)
    __old_turn_round(consensuses)
    __get_old_delegator_info(operators[2], accounts[0])
    turn_round(consensuses)
